...
class Hoadon extends db
{
    ...
    public function doi_trang_thai($trangthai, $id_hd)
    {
        $sql = "UPDATE hoa_don SET trang_thai=$trangthai WHERE id_hd = $id_hd";
        $this->pdo_execute($sql);
    }
    ...
    public function thongke_donhang($thang)
    {
        $sql = "SELECT 
        COUNT(DISTINCT(hoa_don.id_hd)) as tongdonhang,
        COALESCE(SUM(chi_tiet_hoa_don.gia_ban), 0) as tongdoanhthu,
        COALESCE(SUM(chi_tiet_hoa_don.so_luong), 0) as tongspban
    FROM chi_tiet_hoa_don 
    INNER JOIN hoa_don ON chi_tiet_hoa_don.id_hd = hoa_don.id_hd 
    WHERE hoa_don.trang_thai = 5 AND YEAR(hoa_don.ngay_dat) = $thang";

        $result = $this->pdo_query_one($sql);

        // Nếu không có dữ liệu, trả về giá trị mặc định
        return $result ?: ['tongdonhang' => 0, 'tongdoanhthu' => 0, 'tongspban' => 0];
    }

    public function load_thongke_donhang($thang)
    {
        $sql = "SELECT 
        MONTH(hoa_don.ngay_dat) AS thang_dat,
        COUNT(hoa_don.id_hd) AS so_luong_ban,
        COALESCE(SUM(chi_tiet_hoa_don.gia_ban), 0) AS doanh_thu
    FROM hoa_don
    JOIN chi_tiet_hoa_don ON hoa_don.id_hd = chi_tiet_hoa_don.id_hd
    WHERE hoa_don.trang_thai = 5
    AND YEAR(hoa_don.ngay_dat) = $thang
    GROUP BY thang_dat
    ORDER BY thang_dat ASC";
