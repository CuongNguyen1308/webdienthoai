...
@section('content')
                                    ...
                                    <div class="cart-item" data-product-id="{{ $item['id_ctsp'] }}">
                                        <div class="input-group mb-3" style="width: 200px;">
                                            ...
                                            <input type="number" class="form-control text-center quantity"
                                                data-id-gh="{{ $item['id_gh'] }}" data-product-id="{{ $item['id_ctsp'] }}"
                                                value="{{ $item['so_luong'] }}" min="1" style="width: 60px;">
    ...
    <script>
        $(document).on('change', '.form-control[name^="variant_"]', function() {
            ...
            $.ajax({
                url: '{{ route('updateVariant') }}', // File PHP xử lý cập nhật trạng thái
                type: 'POST',
                data: {
                    id_gh: id_gh,
                    id_ctsp: id_ctsp,
